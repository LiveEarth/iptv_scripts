...
This article was made with the help of Google Translate, which means that part of it may have difficult meaning for understanding. Please reread the sentence or the text excerpt again - I'm sure it will become clear after that.
...
| url                                                                                                                                                 | title                           |
+-----------------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------+
| http://83.228.88.***:8***/play/a***                                                                                                                 | BNT1                            |
| http://83.228.88.***:8***/play/a***                                                                                                                 | BNT2                            |
| http://83.228.88.***:8***/play/a***                                                                                                                 | BNT3                            |
+-----------------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------+
```
...
3. Next step is to check for local yourls activity logs. Why this are need? The main goal here is to prevent paid user subscription interuption of iptv channel stream by iptv provider security rules. Most of them have limitation of concurent connections per account (by default the allowed connection is 1). If in the same time we and owner of leaked account use the iptv service, then security module will interrupt our and paid user channel stream. This is indication for illegal using of iptv service and there is option to ban our public ip address. The logic here is if there is interruption on iptv stream (the STB will make a request to our yourls and this will be logged in logs) for less than 1 min go to next steps and get a new account credentials (user, pass). This will include change of channel and interruption of channel stream. If there is no logged activity on yourls, do not do anything and exit from script (this mean that continue to use current account).
4. The next step is to prepare an array with username and password to be checked on iptv provider side. When this are ready choice one key and value and check it to iptv side for concurent connection. If there is more than 0 - choice a new account, if it is 0 - contunue with this account.
If variable getaccountstatus content digit great than 0, pause for 4 seconds and select a new account from array. If the variable getaccountstatus are empty this mean that user are not active and need to be removed from array and also from `rotate-user-pass/accounts` file. After that - contunue to next account. Here at this moment is reason why I make check for count number in `rotate-user-pass/accounts` file (in step 2). Here is some limitation - sometime from iptv provider side return false empty response from checking user concurent connection value in variable getaccountstatus. This produce removing of active user accounts from `rotate-user-pass/accounts` file.
5. Check for specific channel in user subscribtion (for example are there is BG channels or no). If ffprobe was fail - choice new account, if no - contunue with this account. Look for variable checkbgchannelnumb.
6. Prepare the neccesairy file for DB insert for selected user.
7. The last step is to clean up the temp file and array value for used/selected account.

### The conclusion
I made this article for first for myself ( I can not be sure that will remember details for it after few years) and next also for all enthusiasts like me who want to experiment and learn more. Please share you knoledge, explore posibilities around you, make positive changes for you and people around and last but not least - be happy!
